...
For a simple, smoothly flowing fluid, Newton determined that the viscous force ($F$) between two adjacent layers depends directly on three main factors:

1. **Area ($A$):** The surface area of the layers in contact.
2. **Velocity Gradient ($\frac{dv}{dx}$):** The change in velocity ($dv$) between layers relative to the distance ($dx$) separating them.
...
* **$\eta$ (Eta)** is the **coefficient of viscosity**, a property specific to each fluid that measures its resistance to gradual deformation.
